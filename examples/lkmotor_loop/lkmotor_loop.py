...
# ------------------------------------------------------------------
# File Name:        lkmotor_loop.py
# Author:           Han Xudong
# Version:          1.0.0
# Created:          2024/06/05
# Description:      A loop motion example for LK motors.
...
def lkmotor_loop(
    ...
    """Loop motion for LKMotor motors.

    Args:
        com_port: The COM port of the LKMotor controller.
        baud_rate: The baud rate of the LKMotor controller.
    ...
    # Create an instance of the LKMotor class
    lkmotor = LKMotor(com_port, baud_rate, model)

    # Set zero position
    for id in ids:
        lkmotor.set_zero(id_num=id)
    ...
    while counter < rounds:
        # Move to pos_1
        for id in ids:
            lkmotor.set_pos(id_num=id, pos=pos_1, vel=vel)
        while np.mean(np.abs(cur_pos - pos_1)) > err:
            time_list.append(time.time() - start_time)
            for i, id in enumerate(ids):
                c_pos, c_vel = lkmotor.get_posvel(id_num=id)
                cur_pos[i] = c_pos
        ...
        # Move to pos_2
        for id in ids:
            lkmotor.set_pos(id_num=id, pos=pos_2, vel=vel)
    ...
    pos_0 = 0
    for id in ids:
        lkmotor.set_pos(id_num=id, pos=pos_0, vel=vel)
...
if __name__ == "__main__":
    ...
    lkmotor_loop(com_port, baud_rate, model, ids, pos_range, rounds, vel)
